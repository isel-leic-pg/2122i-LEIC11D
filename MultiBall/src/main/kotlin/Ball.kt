 ...
 */
fun Ball.clicked(x: Int, y:Int): Boolean = (Point(x,y)-center).length() <= RADIUS_BALL  //true // TODO

operator fun Point.minus(p: Point) = Delta(x-p.x, y-p.y)

fun Delta.length() = sqrt( (dx*dx + dy*dy).toDouble() ).roundToInt()
